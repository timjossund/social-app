<x-layout :doctitle="$post->title">
  <div class="container py-md-5 container--narrow full-h">
    ...
    <p class="small mb-4 text-muted flex">
      <a href="/profile/{{ $post->user->username }}"><img class="avatar-tiny" src="{{$post->user->avatar}}" /></a> Posted by <a href="/profile/{{ $post->user->username }}">{{ $post->user->username }}</a> on {{$post->created_at->format('n/j/Y')}}
    </p>
